...
impl World {
    ...
    pub fn remove(&mut self, entity: &Entity) {
        if let Some(entity_table_data) = self.get_valid_entity_table_data_mut(entity) {
            ...
            // remove from table
            self.tables.get_mut(table_index).unwrap().remove(inner_table_index);
    ...
    fn add_entity_from_vec(&mut self, components: Vec<Box<dyn DynamicComponent>>) -> Entity {
        ...
        let generation = self.entities.get(index).map(|it| it.generation).unwrap_or(0);
        ...
        let inner_table_index = table.add(entity, &component_set, components);
    ...
    fn get_valid_entity_table_data(&self, entity: &Entity) -> Option<&EntityTableData> {
        match self.entities.get(entity.index).as_ref() {
            Some(entity_table_data) if entity.generation == entity_table_data.generation => Some(entity_table_data),
            _ => None,
        }
    }
...
impl Table {
    ...
    fn add(&mut self, entity: Entity, component_set: &[usize], components: Vec<Box<dyn DynamicComponent>>) -> usize {
        match self.removed.pop() {
            Some(index) => {
                for (component_set_index, component) in components.into_iter().enumerate() {
                    self.collumns[component_set[component_set_index]].as_mut().unwrap()[index] = Some(component)
                }
            ...
            None => {
                for (component_set_index, component) in components.into_iter().enumerate() {
                    self.collumns[component_set[component_set_index]].as_mut().unwrap().push(Some(component))
                }
...
impl<T> DynamicComponent for T where T: Component {
    fn get_id(&self) -> &str {
        T::get_id()
    }
}
...
impl<T: DynamicComponent + 'static, U: DynamicComponent + 'static> ComponentBundle for (T, U) {
    fn into_components(self: Self) -> Vec<Box<dyn DynamicComponent>> {
        vec![Box::new(self.0), Box::new(self.1)]
    }
}

impl<T: DynamicComponent + 'static, U: DynamicComponent + 'static, V: DynamicComponent + 'static> ComponentBundle for (T, U, V) {
    fn into_components(self: Self) -> Vec<Box<dyn DynamicComponent>> {
        vec![Box::new(self.0), Box::new(self.1), Box::new(self.2)]
    }
}

impl<T: DynamicComponent + 'static, U: DynamicComponent + 'static, V: DynamicComponent + 'static, W: DynamicComponent + 'static> ComponentBundle for (T, U, V, W) {
    fn into_components(self: Self) -> Vec<Box<dyn DynamicComponent>> {
